...
int SchemeLWE::decrypt_mod_p(const Ctxt_LWE& ct) const
{
    clock_t start = clock();
    ...
    output = parLWE.mod_q_base(output);
    output = int(round(double(output*Param::p)/double(parLWE.q_base)));
...
double SchemeLWE::noise(const Ctxt_LWE& ct, int m) const
{
    ...
    if (0 == noise)
        return 0;
    return log(abs(noise)) / log(2);
}


double SchemeLWE::noise_mod_p(const Ctxt_LWE& ct, int m) const
{
    int noise = ct.b - round(parLWE.q_base / Param::p) * m;
    for (int i = 0; i < parLWE.n; i++)
    {
        noise += ct.a[i] * sk_base[i];
    }
...
void external_product(vector<long>& res, const vector<int>& poly, const vector<FFTPoly>& poly_vector){
    int l = poly_vector.size();
    int logB = ceil((log(q_boot)/log(2)) / l);
    int B = 1 << logB;

    external_product(res, poly, poly_vector, B, logB, l);
}
