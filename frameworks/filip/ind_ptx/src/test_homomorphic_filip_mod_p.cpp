...
ModQPoly random_poly(int bound) {
  ...
  for (int i = 0; i < Param::N; i++) {
    u[i] = rand() % bound;
    if (bound > 2) {
      if (u[i] * 2 > bound) u[i] -= bound;
    }
  }
  return u;
}

// to normalize the result of a mod operation
// to be in the range [0, p-1]
int normalize(int value, int p) { return ((value % p) + p) % p; }

void printVector(const std::vector<int>& vec) {
  cout << ">>> Here is your vector my lord: [";
...
void test_global_setup(SchemeLWE& s_lwe, bool verbose = false) {
  ...
  homFilip.run_global_setup(
      filip.sk);  // uses FiLIP's secret key to run global setup
  ...
  for (int i = 0; i < len_sk; i++) {
    ...
    for (int j = 1; j < Param::N; j++) assert(0 == dec_msg[j]);
...
void test_p_dependent_setup(SchemeLWE& s_lwe, bool verbose = false) {
  ...
  // Now test if ciphertexts initialized on p-setup are encrypting the expected
  // values
  SKey_boot& sk_ngs = homFilip.fhe.sk_boot;
  ModQPoly dec_msg(Param::N, 0);
...
void test_simulate_homomorphic_filip_dec(SchemeLWE& s_lwe,
                                         bool verbose = false) {
  int BENCHMARK_ITERATIONS = 100;
  Printer printer(true);
  printer.PrintHeader("Homomorphic Filip Test");

  int NTESTS = 1;  // number of times we run the same test with random inputs
...
#ifndef SKE_BENCH_ONLY
  BENCHMARK_ITER("p-dependent HHE setup", BENCHMARK_ITERATIONS,
                 { homFilip.run_p_dependent_setup(p); });
#endif
  for (int _i = 0; _i < NTESTS; _i++) {
    ...
    std::vector<int> ctxt_filip;
    printVector(msg_bin);
    printer.PrintMessages("#elements: ", msg_bin.size(),
                          ", size of element (bit): ", sizeof(msg_bin[0]) * 8);

    BENCHMARK_ITER("SKE.Enc()", BENCHMARK_ITERATIONS,
                   { ctxt_filip = filip.enc(iv, msg_bin); });

#ifndef SKE_BENCH_ONLY
    SKey_boot& sk_boot = s_lwe.sk_boot;

    // ---------- Now run FiLIP decryption homomorphically
    // cout << "Ctxt_LWE ctxt_fhe = homFilip.transform(iv, ctxt_filip);" <<
    Ctxt_LWE ctxt_fhe;
    printer.PrintHeader("Transciphering");
    printer.PrintMessages(
        "#elements: ", ctxt_filip.size(),
        ", size of element (bit): ", sizeof(ctxt_filip[0]) * 8);

    BENCHMARK_ITER("HHE.Decom()", BENCHMARK_ITERATIONS,
                   { ctxt_fhe = homFilip.transform(iv, ctxt_filip); });
    ...
    if (verbose) {
      ...
      cout << "noise budget: log(q_base) - log(p) - noise = "
           << log(parLWE.q_base) / log(2) << " - " << log(p) / log(2) << " - "
           << noise << " = "
           << log(parLWE.q_base) / log(2) - log(p) / log(2) - noise << endl;
    }

    printer.PrintMessages("p:", p, ", logp:", logp);
    printer.PrintMessages("Original Message:", normalize(msg_mod_p, p));
    printer.PrintMessages("Decrypted message:", normalize(dec_msg, p));
    // ----- Now check the final result: the FHE ciphertext must encrypt 2^k *
    // ptxt mod p
