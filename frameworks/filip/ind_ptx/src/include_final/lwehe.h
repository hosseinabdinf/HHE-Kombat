 ...
 * @param[out] res polynomial ciphertext with the result without mod q
 * @param[in] poly polynomial ciphertext
...
class SchemeLWE
{
    ...
    SKey_boot sk_boot;  // made public to simplify homomorphic_filip.cpp
        ...
        /**
         *  ugly hack to decrypt messages with plaintext space defined by Param::p
         */
        int decrypt_mod_p(const Ctxt_LWE& ct) const;
        ...
        /**
         *  ugly hack to get noise considering plaintext space defined by Param::p
         */
        double noise_mod_p(const Ctxt_LWE& ct, int m) const;
