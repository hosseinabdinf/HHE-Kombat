...
#define RING_DIMENSION 1<<10
//#define RING_DIMENSION 1<<11
 ...
 */
inline int lazy_mod(const int input, const int q)
{
    int coef = input % q;
    if (2*coef > q)
        return coef - q;
 ...
 */
#if RING_DIMENSION == 1 << 10
const int q_boot = 912829;     // ~2^19.8, prime
#else
const int q_boot = 1073741827; // ~2^30, prime
#endif
...
{
    public:
        // decomposition base of key-switching keys (l_ksk and Nl are computed as a function of this)
        #if RING_DIMENSION == 1024
        const static int B_ksk = 1 << 3;
        #else
        const static int B_ksk = 1 << 5;
        #endif
       ...
       const static int p = 1 << 4;
        ...
        void init()
        {
            if (scheme_type == SchemeType::NTRU)
            {
                ...
                shift_bsk[1] = 4;
                bsk_partition[0] = 750;
                bsk_partition[1] = n - bsk_partition[0];
            }
