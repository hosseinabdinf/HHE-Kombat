...
HomomorphicFiLIPModP::HomomorphicFiLIPModP(const FiLIP& f, SchemeLWE& s_lwe,
                                           int _log_B_ngs)
    : len_sk(f.len_sk),
      size_subset(f.size_subset),
      size_domain_thr(f.size_domain_thr),
...
void HomomorphicFiLIPModP::run_global_setup(const vector<int>& sk) {
...
#if DEBUG
  double t_duration = duration_cast<milliseconds>(t_after - t_before).count();
  cout << "|--> Time to run client's global setup: " << t_duration << " ms"
       << endl;
#endif
  ...
  cout << "|--> Time to run server's global setup: " << t_duration << " ms"
       << endl;
#endif
}
...
void HomomorphicFiLIPModP::run_p_dependent_setup(int p) {
  ...
  ModQPoly Delta_t =
      get_test_vector_for_threshold_xor(Param::N, threshold_limit);
  Delta_t *= Delta;
  Delta_t %= q_boot;  // Delta * t(X)

  std::vector<long> long_poly(Param::N);

  sca_enc_b_t = std::vector<std::vector<ModQPoly>>(
      logp);  // [i][j] --> enc(2^i*sk[j]*t(X) mod p)
  sca_enc_not_b_t = std::vector<std::vector<ModQPoly>>(
      logp);  // [i][j] --> enc(2^i*NOT(sk[j])*t(X) mod p)
  for (int i = 0; i < logp; i++) {
    ...
    for (int j = 0; j < len_sk; j++) {
      ...
      external_product(long_poly, pow2Delta_t, enc_sk[j], B_ngs, log_B_ngs,
                       l_ngs);  // Delta*2^i*t(X) * enc(sk[j])
      sca_enc_b_t[i][j] = ModQPoly(Param::N);
      mod_q_boot(sca_enc_b_t[i][j], long_poly);

      sca_enc_not_b_t[i][j] = ModQPoly(Param::N);
      external_product(long_poly, pow2Delta_t, enc_not_sk[j], B_ngs, log_B_ngs,
                       l_ngs);  // Delta*2^i*t(X) * enc(NOT(sk[j]))
      sca_enc_not_b_t[i][j] = ModQPoly(Param::N);
...
// std::vector<NGSFFTctxt*> vec_ctxt_xor; // ciphertexts encrypting the bits to
// be xored std::vector<ModQPoly*> ctxt_xor; //  scalar ciphertexts encrypting
// 2^pow_2 * t(X) * b for every bit b be xored std::vector<NGSFFTctxt*> ctxt_hw;
// // encryptions of X^(2*b) for every bit b to be added for the hamming weight
void HomomorphicFiLIPModP::subset_permut_whiten(int pow_2) {
  // set indexes as [0, 1, ..., len_sk - 1]
  vector<int> indexes(len_sk);
  for (int i = 0; i < len_sk; i++) indexes[i] = i;
...
Ctxt_LWE HomomorphicFiLIPModP::transform(long int iv,
  ...
  int n_ints =
      c.size() *
      (len_sk /
       2);  // for each c[i], we need len_sk/2 random integers for permutation
  int modulus = len_sk;
  int n_bits = c.size() * size_subset;  // for each c[i], we need size_subset
                                        // random bits for whitening
...
//	double t_duration = duration_cast<milliseconds>(t_after -
// t_before).count();
#if DEBUG
  double t_duration = duration_cast<microseconds>(t_after - t_before).count();
  cout << "|--> (HomomorphicFiLIPModP) Time to generate random bytes: "
       << t_duration << " microseconds" << endl;
#endif
  ...
  cout << "*** DEBUG mode on (turn it off to measure time more accurately)"
       << endl;
  cout << "|--> Before NTRU->LWE keySwt: " << endl;
  cout << "|--> decypted msg: " << msg[0] << endl;
  cout << "|--> noise: " << noise << endl;
  cout << "|--> noise budget: log(q_boot) - log(p) - noise = "
       << log(q_boot) / log(2) << " - " << log(Param::p) / log(2) << " - "
       << noise << " = "
       << log(q_boot) / log(2) - log(Param::p) / log(2) - noise << endl;
#endif
  ...
  cout << "|--> (HomomorphicFiLIPModP) Time to key switch at the end: "
       << t_duration * 0.001 << " ms" << endl;
#else
  fhe.key_switch(ctxt, scalar_ctxt);
#endif
...
ModQPoly HomomorphicFiLIPModP::enc_bit(int ci, int pow_2) {
  ...
  //     For this, we have an FHE ciphertext encrypting 2^k*F(s), where s is
  //     FiLIP's secret key
  // and we want to output an FHE ciphertext encrypting 2^k * (F(s) xor
  // filip_ctxt), but this is equivalent to outputting an FHE encryption of
  ...
  if (1 == ci) {
    ...
    f_x_y[0] =
        delta_2k -
        f_x_y[0];  // negate the constant term, where the bit is encrypted
    for (int i = 1; i < Param::N; i++) f_x_y[i] *= -1;
    mod_q_boot(f_x_y);
  }
...
ModQPoly HomomorphicFiLIPModP::compute_xor_thr(int pow_2) {
  this->subset_permut_whiten(
      pow_2);  // init vectors vec_ctxt_xor, ctxt_xor, and ctxt_hw
  ...
  ModQPoly ct_xors(*(ctxt_xor[0]));  // copy enc of 2^k * t(X) * b for the first
                                     // bit b to be xored
  ...
  ModQPoly ct_res = lift_to_exponent_scaled(
      ct_xors, u);  // ct_res encrypts 2^k * t(X) * X^XOR(b0,...,bn)

  // now add the bits for the hamming weight
  vector<long> tmp_poly_long(Param::N);
  for (int i = 0; i < size_domain_thr; i++) {
    external_product(tmp_poly_long, ct_res, *(ctxt_hw[i]), B_ngs, log_B_ngs,
                     l_ngs);
    mod_q_boot(ct_res, tmp_poly_long);  // ct_res encrypts 2^k*t(X)*X^(XOR +
                                        // 2*(bits[0]+...+bits[i]))
  }
...
double HomomorphicFiLIPModP::noise(const std::vector<Ctxt_LWE>& c,
                                   const std::vector<int>& m) {
  return 0.0;
}

std::ostream& operator<<(std::ostream& os, const HomomorphicFiLIPModP& u) {
  double final_keyswt_key_MB = parLWE.Nl * (parLWE.n + 1) *
                               (log(parLWE.q_base) / log(2)) / (8.0 * 1000000);
  double client_up_MB =
      u.len_sk * u.l_ngs * Param::N * (log(q_boot) / log(2)) / (8.0 * 1000000);
  os << "|--> HomomorphicFiLIP: {"
     << "p: " << u.p << ", ceil(log p): " << u.logp << ", N: " << Param::N
     << ", B_ngs: 2**" << u.log_B_ngs << ", l_ngs: " << u.l_ngs
     << ", len(Filip.sk): " << u.len_sk << ", size_subset: " << u.size_subset
