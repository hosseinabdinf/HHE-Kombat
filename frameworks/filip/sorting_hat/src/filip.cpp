...
FiLIP::FiLIP(int len_sk, int size_subset, int size_domain_thr,
             int threshold_limit)
    : len_sk(len_sk), size_subset(size_subset),
      size_domain_thr(size_domain_thr), threshold_limit(threshold_limit),
      num_bits_xored(size_subset - size_domain_thr), uni_sampler(0, len_sk) {
...
std::ostream &operator<<(std::ostream &os, const FiLIP &u) {
  os << "FiLIP: {"
     << "len_sk: " << u.len_sk << ", size_subset: " << u.size_subset
