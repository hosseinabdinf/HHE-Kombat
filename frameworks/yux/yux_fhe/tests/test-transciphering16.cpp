...
int main(int argc, char **argv) {
  cout << "-----------Begin-------\n";
  // ArgMapping amap;
  //            security Level/2，L, c, p,  d=16/24,  s=预计的槽数，
  //            chosen_m=0，false)
  // long FindM(long k, long L, long c, long p, long d, long s, long chosen_m,
  // bool verbose)
  long idx = 3;
  ...
  cout << "-----Test_Sym: c=" << c << ", m=" << m << ", bits =" << bits << endl;
  ...
  { // IndexSet allPrimes(0,context.numPrimes()-1);
    ...
    cout << "-----" << context.numPrimes()
         << " primes , total bitsize=" << context.logOfProduct(allPrimes)
         << ", security level: " << context.securityLevel() << endl;

    myfile << "[ m = " << m << ", p = " << p << ", c = " << c
           << ", bits = " << bits << ", nRounds = " << ROUND << "]\n"
  ...
  cout << "-----" << context.getZMStar().getNSlots() << " slots ("
       << (context.getZMStar().getNSlots())
       << " blocks total), ord(p) = " << (context.getZMStar().getOrdP())
       << "\n";
  if (packed)
    cout << ". x" << e << " ctxts";
  cout << endl;
...
/*---Test END----*/
  ...
  if (ptxt != tmpBytes) {
    ...
    cout << "Homomorphic symmtric decryption correct!! Done [glasped.count()] "
            "in "
         << glasped.count() << " ms" << endl;
    cout << "Homomorphic symmtric decryption correct!! Done in " << tm
         << " seconds\n";
    //  Write file
    // myfile << "【homEncrypted】:  " << homEncrypted << "\n\n";
    {
      myfile << "Homomorphic symmtric decryption correct!! done in " << tm
             << " seconds\n\n";
      myfile << "【After homomorphic decrypt.length】  " << tmpBytes.length()
             << "\n\n";
      myfile << "  input symCtxt = ";
      myfile << "[";
      for (long i = 0; i < symCtxt.length() && i < 32; i++)
        myfile << std::hex << std::setw(2) << (long)symCtxt[i] << " ";
      if (tmpBytes.length() > 32)
        myfile << "...";
      myfile << "]" << endl;
      myfile << "output tmpBytes = ";
      myfile << "[";
      for (long i = 0; i < tmpBytes.length() && i < 32; i++)
        myfile << std::hex << std::setw(2) << (long)tmpBytes[i] << " ";
      if (tmpBytes.length() > 32)
        myfile << "...";
      myfile << "]" << endl;
      myfile << " should be ptxt = ";
      myfile << "[";
      for (long i = 0; i < ptxt.length() && i < 32; i++)
        myfile << std::hex << std::setw(2) << (long)ptxt[i] << " ";
      if (ptxt.length() > 32)
        myfile << "...";
      myfile << "]" << endl;
    }
    cout << "-------After homomorphic decrypt and DecodeTo16Ctxt's length:   "
         << tmpBytes.length() << "\n";
    cout << "  input symCtxt = ";
    printState(symCtxt);
    cout << endl;
