...
void Transcipher16::encodeToKeysForExpand(
    ZZX &encData, const Vec<uint8_t> &data, const int32_t len,
...
void Transcipher16::encryptSymKeyForExpand(
    Ctxt &eKey, Vec<uint8_t> &symKey, const PubKey &hePK,
  ...
  KeyExpansion(encRoundKeySchedule, ROUND, BlockByte,
               symKey.data()); // symKey.length() =16
  // Decrypt roundkey
  ...
  ZZX encoded;
  encodeToKeysForExpand(encoded, expanded, length_s,
                        ea); // encode as HE plaintext
...
void fasterSingleRoundKeyFor1920(Ctxt &ekey, vector<Ctxt> &temp,
                                 const Ctxt &input,
                                 const EncryptedArrayDerived<PA_GF2> &ea,
                                 const int32_t slots,
                                 const int32_t logOfSlots) {
...
void fasterSingleRoundKeyFor960(Ctxt &ekey, vector<Ctxt> &temp,
                                const Ctxt &input,
                                const EncryptedArrayDerived<PA_GF2> &ea,
                                const int32_t slots, const int32_t logOfSlots) {
  // rotate (0, 1, 2, 4, 8, ..., 2^7)
  temp[0] = input;
  for (size_t i = 1; i <= 4; i++) {
    temp[i] = temp[i - 1];
...
//
void Transcipher16::handleSingleRoundKey(
    Ctxt &ekey, const Ctxt &input, const PubKey &hePK,
    const EncryptedArrayDerived<PA_GF2> &ea) {
...
void Transcipher16::handleRoundKeyForThreads(
    vector<Ctxt> &ekey, const Ctxt &input, const PubKey &hePK,
...
void Transcipher16::handleRoundKey(vector<Ctxt> &ekey, const Ctxt &input,
                                   const PubKey &hePK,
                                   const EncryptedArrayDerived<PA_GF2> &ea,
                                   const int32_t len) {
