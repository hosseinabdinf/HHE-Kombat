...
shared_ptr<Context> Transcipher16_F_p::create_context(uint64_t m, uint64_t p,
...
// int Transcipher16_F_p::print_noise() { return
...
helib::EncryptedArray Transcipher16_F_p::getEa() { return ea; }

//----------------------------------------------------------------
// encrypt the expanded key roundKeySchedule.
void Transcipher16_F_p::encryptSymKey(vector<Ctxt> &eKey,
...
void Transcipher16_F_p::FHE_YuxDecrypt(vector<Ctxt> &eData,
...
// packing
void Transcipher16_F_p::encodeTo16Ctxt(Vec<ZZX> &encData,
                                       const Vec<uint64_t> &data, long s) {
  // 一个分组有16个字
  long nAllBlocks = divc(data.length(),
                         BlockWords); // ceil( data.length()/16 ) =(a + b - 1)/b
...
void Transcipher16_F_p::decSboxFunc(vector<Ctxt> &eData, long begin,
                                    Ctxt &encA) {
  Ctxt c0(eData[begin]);
