...
void Transcipher1::encryptSymKey(vector<Ctxt> &eKey, Vec<uint8_t> &symKey,
                                 const PubKey &hePK,
                                 const EncryptedArrayDerived<PA_GF2> &ea,
                                 bool key2dec) {
  // Round Key length
...
void Transcipher1::buildLinEnc(vector<PolyType> &encLinTran,
  ...
  // corresponding to bytes 3,7,11,15 of each blocks encLinTran[1]: The
  // constants only have zero entires in their slots corresponding to bytes
  // 0,4,8,12 of each blocks, others is 1;
...
void Transcipher1::decSboxFunc(Ctxt &c, vector<PolyType> encLinTran, Ctxt &encA,
                               const EncryptedArrayDerived<PA_GF2> &ea) {
...
void Transcipher1::decSboxFunc2(Ctxt &c, vector<PolyType> encLinTran,
                                Ctxt &encA,
                                const EncryptedArrayDerived<PA_GF2> &ea) {
  // The basic rotation amount along the 1st dimension
  long rotAmount = ea.getContext().getZMStar().OrderOf(0) / 16;
...
void Transcipher1::Linear_function(Ctxt &c,
                                   const EncryptedArrayDerived<PA_GF2> &ea) {
  // The basic rotation amount along the 1st dimension
  long rotAmount = ea.getContext().getZMStar().OrderOf(0) / 16;
...
void Transcipher1::homSymDec(vector<Ctxt> &eData, const vector<Ctxt> &symKey,
  ...
  encLinTran.resize(3,
                    DoubleCRT(ea.getContext(), ea.getContext().fullPrimes()));
