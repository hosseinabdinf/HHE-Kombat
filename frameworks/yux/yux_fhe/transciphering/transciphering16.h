...
public:
  ...
  void buildRoundConstant(Ctxt &encA, const EncryptedArrayDerived<PA_GF2> &ea);

  // run the Yux key-expansion and then encrypt the expanded key.
  void encryptSymKey(vector<Ctxt> &eKey, Vec<uint8_t> &symKey,
                     const PubKey &hePK,
  ...
  /**
   * The blow four function help to expand a single ciphertext to multiple
   * ciphertexts who encrypt the round key of block cipher.
   */

  // Encode  plaintext/ciphertext bytes as native HE plaintext
  // packing
  // only for ea.size() > 240, encode to a single ciphertext
  void encodeToKeysForExpand(ZZX &encData, const Vec<uint8_t> &data,
                             const int32_t len,
                             const EncryptedArrayDerived<PA_GF2> &ea);

  // run the Yux key-expansion and then encrypt the expanded key.
  void encryptSymKeyForExpand(Ctxt &eKey, Vec<uint8_t> &symKey,
                              const PubKey &hePK,
                              const EncryptedArrayDerived<PA_GF2> &ea,
                              bool key2dec);

  static void handleSingleRoundKey(Ctxt &ekey, const Ctxt &input,
                                   const PubKey &hePK,
                                   const EncryptedArrayDerived<PA_GF2> &ea);

  static void handleRoundKeyForThreads(std::vector<Ctxt> &ekey,
                                       const Ctxt &input, const PubKey &hePK,
                                       const EncryptedArrayDerived<PA_GF2> &ea,
                                       const int32_t len, const uint8_t data,
                                       const size_t first, const size_t last);

  void handleRoundKey(std::vector<Ctxt> &ekey, const Ctxt &input,
                      const PubKey &hePK,
                      const EncryptedArrayDerived<PA_GF2> &ea,
                      const int32_t len);
};
