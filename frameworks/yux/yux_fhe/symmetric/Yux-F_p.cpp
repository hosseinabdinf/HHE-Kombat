...
void Yux_F_p::subtractRoundKey(uint64_t state[], uint64_t RoundKey[],
                               int round) {
  int i;
  for (i = 0; i < BlockWords; i++) {
    int key_id = BlockWords * round + i;
...
void Yux_F_p::decLinearLayer(uint64_t in[16]) {
  ...
  // invert linear Layer
  for (j = 0; j < 16; j++) {
    in[j] = (temp[j] + temp[(j + 3) % 16] + temp[(j + 4) % 16] +
             temp[(j + 8) % 16] + temp[(j + 9) % 16] + temp[(j + 12) % 16] +
             temp[(j + 14) % 16]) %
            modulus;
  }
}
...
void Yux_F_p::encSboxFi(uint64_t state[], int begin) {
  ...
  uint64_t temp =
      (modulus - (c0 * c1 + c2 + roundConstant + (modulus - c3)) % modulus) %
      modulus;
    ...
    in[j] =
        (((temp[(j) % 16] + temp[(j + 4) % 16]) * 9363) % modulus +
         ((temp[(j + 1) % 16] + temp[(j + 2) % 16] + temp[(j + 3) % 16] +
           ...
           temp[(j + 13) % 16] + temp[(j + 14) % 16] + temp[(j + 15) % 16]) *
          53054) %
             modulus +
         ((temp[(j + 8) % 16] + temp[(j + 12) % 16]) * 9362) % modulus +
         ((temp[(j + 9) % 16] + temp[(j + 10) % 16] + temp[(j + 11) % 16]) *
          53053) %
             modulus) %
        modulus;
  }
...
//     others are the roundkey with inverse transformation of the linear
//     function.
