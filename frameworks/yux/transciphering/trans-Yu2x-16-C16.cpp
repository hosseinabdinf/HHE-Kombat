...
int Trans_Yu2x_16_16::print_noise(vector<Ctxt>& ciphs) {
...
// Encode  plaintext/ciphertext bytes as native HE plaintext
// packing
void Trans_Yu2x_16_16::encodeTo32Ctxt(Vec<ZZX>& encData,
                                      const Vec<uint8_t>& data,
                                      const EncryptedArrayDerived<PA_GF2>& ea) {
  // 一个分组有16个字节
  long nBytes = NkGF16;
  long nAllBlocks =
      divc(data.length(), nBytes);  // ceil( data.length()/16 ) =(a + b - 1)/b
...
void Trans_Yu2x_16_16::decodeTo32Ctxt(Vec<uint8_t>& data,
                                      const Vec<ZZX>& encData,
                                      const EncryptedArrayDerived<PA_GF2>& ea) {
  // Check the size of the data array
  long nAllBytes =
      encData.length() * ea.size() * 2;  // total number of input bytes
...
void Trans_Yu2x_16_16::buildRoundConstant(
    Ctxt& encA, const EncryptedArrayDerived<PA_GF2>& ea) {
  // char --> GF2X --> ZZX -->Ctxt
  GF2X polyConstant;
...
void Trans_Yu2x_16_16::encryptSymKey(vector<Ctxt>& eKey,
                                     unsigned char encRoundKeySchedule[],
                                     const PubKey& hePK,
                                     const EncryptedArrayDerived<PA_GF2>& ea,
                                     bool key2dec) {
  // Round Key length
  ...
  // KeyExpansion(encRoundKeySchedule,ROUND,BlockWords*2, symKey.data()); //
  // symKey.length() =16

  Vec<GF2E> keySchedule;
  keySchedule.SetLength(NkGF16 * (ROUND + 1));
  for (int i = 0; i < NkGF16 * (ROUND + 1); i++) {
    unsigned char tmp[2] = {encRoundKeySchedule[2 * i],
                            encRoundKeySchedule[2 * i + 1]};
...
void Trans_Yu2x_16_16::decSboxFunc(vector<Ctxt>& eData, long begin,
                                   Ctxt& encA) {
  Ctxt c0(eData[begin]);
...
void Trans_Yu2x_16_16::homYuxDec(vector<Ctxt>& eData,
                                 const vector<Ctxt>& symKey, Ctxt& encA) {
  ...
  for (long j = 0; j < (long)eData.size(); j++)
    eData[j] += symKey[j];  // initial key addition
...
void Trans_Yu2x_16_16::homSymDec(vector<Ctxt>& eData,
...
void Trans_Yu2x_16_16::encSboxFunc(vector<Ctxt>& eData, long begin,
                                   Ctxt& encA) {
  Ctxt c0(eData[begin]);
  Ctxt c1(eData[begin + 1]);
  Ctxt c2(eData[begin + 2]);
...
void Trans_Yu2x_16_16::encSboxFuncV2(vector<Ctxt>& eData, long begin,
                                     Ctxt& encA) {
  Ctxt c0(eData[begin]);
  Ctxt c1(eData[begin + 1]);
  Ctxt c2(eData[begin + 2]);
...
void Trans_Yu2x_16_16::homSymEnc(vector<Ctxt>& eData,
                                 const vector<Ctxt>& symKey, Ctxt& encA) {
  if (1 > (long)eData.size() || 1 > (long)symKey.size()) return;  // no data/key
  //  long lvlBits = eData[0].getContext().bitsPerLevel;
