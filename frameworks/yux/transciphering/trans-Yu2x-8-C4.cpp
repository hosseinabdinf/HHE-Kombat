...
int Transcipher4::print_noise(vector<Ctxt>& ciphs) {
      ...
      for (long j = 0; j < blocksPerCtxt;
        ...
        for (long k = 0; k < 4; k++) {  // k is the byte number in this block
          ...
          if (byteIdx < data.length()) {
            ...
            GF2XFromBytes(slots[slotIdx], &data[byteIdx],
                          1);  // copy byte as poly
          }
        }
      }
...
void Transcipher4::decodeTo4Ctxt(Vec<uint8_t>& data, const Vec<ZZX>& encData,
  ...
  if (data.length() <= 0 || data.length() > nBytes) data.SetLength(nBytes);
  long nAllBlocks = divc(data.length(), 16);     // ceil( data.length()/16 )
  ...
  for (long group = 0; group < nCtxt; group++) {
    for (long s = 0; s < 4; s++) {
      ...
      for (long j = 0; j < blocksPerCtxt;
        ...
        for (long k = 0; k < 4; k++) {  // k is the byte number in this block
          long byteIdx =
              blockBegin + j * 16 + k * 4 + s;  // column orded within block
          if (byteIdx < data.length()) {
            long slotIdx = j + k * blocksPerCtxt;
...
void Transcipher4::encryptSymKey(vector<Ctxt>& eKey, Vec<uint8_t>& symKey,
                                 const PubKey& hePK,
                                 const EncryptedArrayDerived<PA_GF2>& ea,
                                 bool key2dec) {
  // Round Key length
  ...
  long nRoundKeys =
      KeyExpansion(encRoundKeySchedule, ROUND, BlockByte, symKey.data());
  if (key2dec) {
...
void Transcipher4::decSboxFunc(vector<Ctxt>& eData, long begin, Ctxt& encA,
                               const EncryptedArrayDerived<PA_GF2>& ea) {
...
void Transcipher4::decLinearFunc(vector<Ctxt>& eData, long begin,
                                 const EncryptedArrayDerived<PA_GF2>& ea) {
  // The basic rotation amount along the 1st dimension
  ...
  eData[begin] += in[3];
  Ctxt c4(in[0]);
  ea.rotate1D(c4, 0, 3 * rotAmount);
  eData[begin] += c4;
  Ctxt c8(in[0]);
  ea.rotate1D(c8, 0, 2 * rotAmount);
  eData[begin] += c8;
  Ctxt c9(in[1]);
  ea.rotate1D(c9, 0, 2 * rotAmount);
  eData[begin] += c9;
  Ctxt c12(in[0]);
  ea.rotate1D(c12, 0, 1 * rotAmount);
  eData[begin] += c12;
  Ctxt c14(in[2]);
  ea.rotate1D(c14, 0, 1 * rotAmount);
  eData[begin] += c14;

  // 1 5 9 13
  eData[begin + 1] += c4;
  Ctxt c5(in[1]);
  ea.rotate1D(c5, 0, 3 * rotAmount);
  eData[begin + 1] += c5;
  eData[begin + 1] += c9;
  Ctxt c10(in[2]);
  ea.rotate1D(c10, 0, 2 * rotAmount);
  eData[begin + 1] += c10;
  Ctxt c13(in[1]);
  ea.rotate1D(c13, 0, 1 * rotAmount);
  eData[begin + 1] += c13;
  Ctxt c15(in[3]);
  ea.rotate1D(c15, 0, 1 * rotAmount);
  eData[begin + 1] += c15;

  // 2 6 10 14
  eData[begin + 2] += c5;
  Ctxt c6(in[2]);
  ea.rotate1D(c6, 0, 3 * rotAmount);
  eData[begin + 2] += c6;
  eData[begin + 2] += c10;
  Ctxt c11(in[3]);
  ea.rotate1D(c11, 0, 2 * rotAmount);
  eData[begin + 2] += c11;
  eData[begin + 2] += c14;
  eData[begin + 2] += in[0];
...
void Transcipher4::homSymDec(vector<Ctxt>& eData, const vector<Ctxt>& symKey,
  ...
  // for (long j=0; j<(long)eData.size(); j++) eData[j] += symKey[0];  //
  // initial key addition apply the symmetric rounds (long)symKey.size() cout <<
  // "eData.size() = " << eData.size() << "\n"; cout << "symKey.size() = " <<
  // symKey.size() << "\n";
  ...
  for (long i = 1; i < ROUND; i++) {
    for (long j = 0; j < (eData.size() / 4); j++) {
      // S Layer
      for (long step = 0; step < 4; step++) decSboxFunc(eData, j * 4, encA, ea);
...
void Transcipher4::encSboxFunc(vector<Ctxt>& eData, long begin, Ctxt& encA,
                               const EncryptedArrayDerived<PA_GF2>& ea) {
  // The basic rotation amount along the 1st dimension
  Ctxt c0(eData[begin]);
  Ctxt c1(eData[begin + 1]);
...
void Transcipher4::encLinearFunc(vector<Ctxt>& eData, long begin,
                                 const EncryptedArrayDerived<PA_GF2>& ea) {
  // The basic rotation amount along the 1st dimension
  long rotAmount = ea.getContext().getZMStar().OrderOf(0) / 4;
...
void Transcipher4::homSymEnc(vector<Ctxt>& eData, const vector<Ctxt>& symKey,
  ...
  for (long i = 1; i < ROUND; i++) {
    for (long j = 0; j < (eData.size() / 4); j++) {
      // S Layer
      for (long step = 0; step < 4; step++) encSboxFunc(eData, 4 * j, encA, ea);
