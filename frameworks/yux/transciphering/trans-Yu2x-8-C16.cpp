...
int Transcipher16::print_noise(vector<Ctxt>& ciphs) {
...
void Transcipher16::buildRoundConstant(
...
void Transcipher16::encryptSymKey(vector<Ctxt>& eKey, Vec<uint8_t>& symKey,
                                  const PubKey& hePK,
                                  const EncryptedArrayDerived<PA_GF2>& ea,
                                  bool key2dec) {
  // Round Key length
  ...
  for (long i = 1; i < ROUND; i++) {
    ...
    // 定义线程的 id 变量，多个变量使用数组
    {
      ...
      for(int j = 0; j < NUM_THREADS; ++j)
      {
        ...
        int ret = pthread_create(&threads[j], NULL, decSbox_Pre, (void
    *)&td[j]); if (ret != 0)
        {
...
void Transcipher16::encSboxFuncFrobenius(vector<Ctxt>& eData, long begin,
                                         Ctxt& encA) {
  Ctxt c0(eData[begin]);
  Ctxt c1(eData[begin + 1]);
  Ctxt c2(eData[begin + 2]);
