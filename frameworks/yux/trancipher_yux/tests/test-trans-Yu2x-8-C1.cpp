...
int main(int argc, char **argv) {
    ...
    printer.PrintMessages("Test Symmetric: [", "c=", c, ", packed=", packed, ", m=", m, ", Round=", ROUND, "]");
    ...
    tm += GetTime();
    printer.PrintFormatted("Time: %d seconds\n", tm);

    //  context.getZMStar().printout();
    {
        IndexSet allPrimes(0, context.numPrimes() - 1);
        printer.PrintMessages("HE context:",
            " #p=", context.numPrimes(),
    ...
    */
    printer.PrintMessages("HE context:",
		    "#slots=", context.getZMStar().getNSlots(),
		    "#blocks=", context.getZMStar().getNSlots()/8,
		    "red(p)=", context.getZMStar().getOrdP()
		    );
    if (packed){
	    printer.PrintMessages("#ctxs: ", e);
	    //cout << ". x" << e << " ctxts";
	    //cout << endl;
    }
    ...
    */
    ...
    Transcipher1 trans;
#ifdef homDec
    printer.PrintMessage("Computing symmetric round key tables...");
    tm = -GetTime();
    bool key2dec = true;
    ...
    if (ptxt != tmpBytes) {
        ...
        printer.PrintFormatted("Time: %d (ms), %d seconds\n", glasped.count(), tm);
        //  Write file
