...
#ifndef CPP_IMPL_RKSKNOPRECOMP_H
#define CPP_IMPL_RKSKNOPRECOMP_H

#include "RLWEKeyswitchInterface.h"

class RKSKNoPrecomp : public RKSKInterface {

public:

    RKSKNoPrecomp(std::shared_ptr<RingGSWCryptoParams> params, uint64_t L_bits, uint64_t L_LWE_bits, uint64_t n_LWE, uint32_t max_ct);

    RKSKNoPrecomp(std::shared_ptr<RingGSWCryptoParams> params, uint64_t L_bits, uint32_t max_ct);
...
#endif //CPP_IMPL_RKSKNOPRECOMP_H
