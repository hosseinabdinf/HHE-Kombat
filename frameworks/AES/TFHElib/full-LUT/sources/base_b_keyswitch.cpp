...
void BaseBExtra::CreateKeySwitchKey_fromArray(TLweSample*** result,
                                              const TLweKey* out_key,
                                              const double out_alpha,
                                              const int* in_key, const int n,
                                              const int t, const int basebit,
                                              const int mod) {
      ...
      for (int k = 0; k < mod; k++) {
        for (int ind = 0; ind < N; ind++) {
          if (ind >= k * N_B && ind < (k + 1) * N_B) {
            x->coefsT[ind] =
                in_key[i] * (int32_t(1) << (32 - (j + 1) * basebit));
...
/**
 * translates the message of the result sample by -sum(a[i].s[i]) where s is the
 * secret embedded in ks.
...
void BaseBExtra::KeySwitchTranslate_fromArray(TLweSample* result,
                                              const TLweSample*** ks,
                                              const TLweParams* params,
                                              const int32_t* ai, const int n,
                                              const int t, const int basebit) {
  const int base = 1 << basebit;
  const int32_t prec_offset = int32_t(1)
                              << (32 - (1 + basebit * t));  // precision
...
void BaseBExtra::CreateKeySwitchKey(BaseBKeySwitchKey* result,
                                    const LweKey* in_key,
                                    const TLweKey* out_key) {
  ...
  CreateKeySwitchKey_fromArray(result->ks, out_key, out_key->params->alpha_min,
                               in_key->key, n, t, basebit, b);
}

void BaseBExtra::KeySwitch(TLweSample* result, const BaseBKeySwitchKey* ks,
  ...
  BaseBExtra::KeySwitchTranslate_fromArray(result, (const TLweSample***)ks->ks,
                                           params, sample->a, n, t, basebit);
}

// Keyswitch adapted for base b

void BaseBExtra::KeySwitch_Id(TLweSample* result, const BaseBKeySwitchKey* ks,
  ...
  if (samples.size() > mod) {
    std::cout << "ERROR: Number of inputs for the keyswitch not right"
              << std::endl;
    return;
  }
  // printf("here one ...\n");
  ...
  BaseBExtra::KeySwitchTranslate_fromArray_Generic(
      result, (const TLweSample***)ks->ks, params, (const int32_t**)as, n, t,
      basebit, samples.size());
...
void BaseBExtra::KeySwitchTranslate_fromArray_Generic(
    TLweSample* result, const TLweSample*** ks, const TLweParams* params,
    const int32_t** as, const int n, const int t, const int basebit,
    const int M) {
  const int base = 1 << basebit;
  const int32_t prec_offset = int32_t(1)
                              << (32 - (1 + basebit * t));  // precision
