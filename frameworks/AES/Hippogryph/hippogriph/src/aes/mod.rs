...
use self::{
    aes_utils::{key_expansion, pretty_print_nibbles, u8_to_vec_bool, vec_bool_to_u8},
    casts::{decomposer, recomposer},
...
impl AESStateBoolean {
    pub fn tfhe_encryption_bits(m: &Vec<u64>, client_key: &ClientKey) -> Self {
        ...
        Self {
            bits: m
                .iter()
                .map(|b| client_key.encrypt_arithmetic(*b, &parity_encoding))
                .collect(),
        }
    }
...
pub fn demo_aes() {
    print_header("Hyppogriph AES Demo");
    ...
    let plaintext_bits = plaintext
        .iter()
        .map(|byte| u8_to_vec_bool_integer(*byte))
        .collect::<Vec<Vec<u64>>>()
        .concat();
    ...
    let mut state = AESStateBoolean { bits: Vec::new() };

    benchmark("TFHE Encryption of the plaintext", 100, || {
        state = AESStateBoolean::tfhe_encryption_bits(&plaintext_bits, &client_key);
    });
    // let state = AESStateBoolean::tfhe_encryption_bits(&plaintext_bits, &client_key);

    let mut result = AESStateBoolean { bits: Vec::new() };
