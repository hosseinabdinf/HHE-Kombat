...
use crate::aes::{
    aes_utils::pretty_print_clear,
    clear::{clear_post_circuit, clear_pre_circuit, clear_s_box_boyar},
};
use rayon::iter::{IntoParallelIterator, ParallelIterator};
use tfhe::gadget::{client_key, prelude::*};

use self::benchmark_utils::{benchmark, print_header, print_message, print_status};
use self::{
    aes_utils::{key_expansion, pretty_print_encrypted, u8_to_vec_bool, vec_bool_to_u8},
    clear::clear_mixcolumns,
    inner_boyar_sbox::NonLinearSBox,
    inner_boyar_sbox::NonLinearSboxState,
    linear_circuit::LinearCircuit,
};
...
impl AESState {
    pub fn tfhe_encryption_bits(m: &Vec<bool>, client_key: &ClientKey) -> Self {
        ...
        Self {
            bits: m
                .iter()
                .map(|b| client_key.encrypt_boolean(*b, &parity_encoding))
                .collect(),
        }
    }
    ...
    pub fn tfhe_decryption_float_over_the_torus(&self, client_key: &ClientKey) -> Vec<f64> {
        self.bits
            .iter()
            .map(|c| client_key.decrypt_float_over_the_torus(c))
            .collect()
    }
...
fn add_round_key(state: &AESState, round_key: &Vec<bool>, server_key: &ServerKey) -> AESState {
    ...
    AESState {
        bits: state
            ...
            .map(|(c, k)| {
                if *k {
                    server_key.simple_plaintext_sum(c, 1, 2)
                } else {
                    c.to_owned()
                }
            })
...
fn sub_bytes(state: &AESState, server_key: &ServerKey, client_key_debug: &ClientKey) -> AESState {
    // Slicing in 16 bytes:
    let clear_debug: Vec<bool> = state
        .bits
        .iter()
        .map(|c| client_key_debug.decrypt(c) == 1)
        .collect();
    AESState {
        bits: (0..16)
            .map(|i| (i, state.bits[i * 8..(i + 1) * 8].to_vec()))
            .map(|(i, slice)| {
                let mut pre_circuit = LinearCircuit::new(&slice);
                pre_circuit.execute_circuit(
                    &server_key,
                    "./src/aes/data/pre_circuit.txt",
                    client_key_debug,
                );
                ...
                // println!("////////////////////////");
                let mut non_linear_state = NonLinearSboxState::new(
                    current,
                    "./src/aes/data/gadgets_non_linear.txt",
                    &server_key,
                    client_key_debug,
                );
                // println!("Debug de subbytes : ");
                ...
                let current =
                    non_linear_state.extract_and_cast_output(&server_key, client_key_debug);

                let mut post_circuit = LinearCircuit::new(&current);
                post_circuit.execute_circuit(
                    &server_key,
                    "./src/aes/data/post_circuit.txt",
                    client_key_debug,
                );
                post_circuit.y
            })
...
fn mix_columns(state: &AESState, server_key: &ServerKey, client_key_debug: &ClientKey) -> AESState {
    AESState {
        bits: (0..4)
            .map(|col| {
                let mut circuit =
                    LinearCircuit::new(&state.bits[col * 32..(col + 1) * 32].to_vec());
                circuit.execute_circuit(
                    &server_key,
                    "./src/aes/data/mixcolumns2.txt",
                    &client_key_debug,
                );
                circuit.y
            })
...
pub fn run_aes(
    ...
    //9 full rounds
    for r in 0..9 {
        print_status(format!("Round {}", r + 1).as_str());
...
pub fn test_parameters_for_aes() {
    ...
    let gadget_cast = Gadget::new(
        vec![BooleanEncoding::new_canonical(1, 2)],
        BooleanEncoding::new_canonical(1, 2),
        BooleanEncoding::new(HashSet::from([10]), HashSet::from([1]), 11),
        1,
        &|x| x[0],
    );
    ...
    sbox.gadgets
        .values()
        .for_each(|g| g.test_full(&client_key, &server_key));
}

pub fn demo_aes() {
    print_header("AES Demo");
    ...
    let plaintext_bits = plaintext
        .iter()
        .map(|byte| u8_to_vec_bool(*byte))
        .collect::<Vec<Vec<bool>>>()
        .concat();
    let aes_key_bits = aes_key
        .iter()
        .map(|byte| u8_to_vec_bool(*byte))
        .collect::<Vec<Vec<bool>>>()
        .concat();

    print_message("Encryption of Plaintext using TFHE!");
    print_message(format!("Plaintext size: {:?} bits", plaintext_bits.len()).as_str());
    print_message(format!("Key size: {:?} bits", aes_key_bits.len()).as_str());

    let mut state = AESState { bits: Vec::new() };

    benchmark("TFHE Encryption of the plaintext", 100, || {
        state = AESState::tfhe_encryption_bits(&plaintext_bits, &client_key);
    });

    let mut result = AESState { bits: Vec::new() };

    benchmark("Transciphering", 10, || {
        result = run_aes(&state, &server_key, aes_key_bits.clone(), &client_key);
    });
    ...
    (0..16).for_each(|i| {
        print!(
            "{:02x} ",
            vec_bool_to_u8(&result_clear[i * 8..(i + 1) * 8].to_vec())
        )
    });
    println!();
}
