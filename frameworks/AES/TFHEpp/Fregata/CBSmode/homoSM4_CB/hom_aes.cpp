...
// template <class P>
void MakeSBoxTable(std::vector<TRLWE_1> &Table,
                   const TFHEpp::Key<privksP::targetP> &key) {
  // dec-> bit
  ...
  for (int k = 0; k < 2; k++) {
    ...
    for (int i = 0; i < 128; i++) {
      for (int j = 0; j < 8; j++) {
        poly[i * 8 + j] =
            (typename privksP::targetP::T)Sbox_binary[k * 128 + i][j];
        // cout << Sbox_binary[k * 128 + i][j] << " ";
      }
      // cout << endl;
    }

    Table[k] = TFHEpp::trlweSymIntEncrypt<privksP::targetP>(
        poly, privksP::targetP::alpha, key);
  }
}

void CipherSubBytesMixedPacking(
    TRLWE_1 &result, std::vector<TRLWE_1> &Table,
    std::vector<TFHEpp::TRGSWFFT<typename privksP::targetP>> &select) {
  // last bit
  //  TFHEpp::TRGSWFFT<TRLWE_1> select1;
  // TFHEpp::TRLWE<typename privksP::targetP> resultOfCMUX;
  TFHEpp::CMUXFFT<typename privksP::targetP>(result, select[7], Table[1],
                                             Table[0]);
...
void CipherAddRoundKey(std::vector<std::vector<TLWE_0>> &cipher,
                       std::vector<std::vector<TLWE_0>> &rk, int round) {
  for (int i = 0; i < 16; i++) {
    XOR_Two(cipher[i], cipher[i], rk[round * 16 + i]);
  }
}

void CipherShiftRows(std::vector<std::vector<TLWE_0>> &cipher,
...
void CipherMixColumns(std::vector<std::vector<TLWE_0>> &cipher,
                      std::vector<TLWE_0> &consByte) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    XOR_Four(Tmp, cipher[4 * i + 0], cipher[4 * i + 1], cipher[4 * i + 2],
             cipher[4 * i + 3]);

    // Tm = state[0][i] ^ state[1][i];
    for (int j = 0; j < 8; j++) {
      TFHEpp::HomCOPY<typename bkP::domainP>(
          Tm[j], cipher[4 * i + 0][j]);  // t = cipher[0][i]
    }
...
int main() {
  ...
  printer.PrintHeader("TFHE AES - Fregata");
  ...
  cout << endl;
  BENCHMARK("Make SBox Table", {
    MakeSBoxTable(Table,
                  sk->key.get<privksP::targetP>());  // 利用level1的trlwe key
  });
  ...
  cout << "-> #Plaintexts: " << 16
       << ", Size(plaintext[0]):" << sizeof(plain[0]) * 8 << " bits" << endl;

  std::vector<TLWE_0> consByte(8);

  for (int i = 0; i < 8; i++) {
    // encrypt 0
    consByte[i] = TFHEpp::tlweSymIntEncrypt<typename bkP::domainP>(
        (typename bkP::domainP::T)byte_mul2[i], bkP::domainP::alpha,
        sk->key.get<typename bkP::domainP>());
  }

  printer.PrintMessage("Round Key Generation...");
  ...
  BENCHMARK("AES Key Expansion",
            { nRoundKeys = AESKeyExpansion(RoundKey, aeskey, 128); });

  printer.PrintMessages("#Rounds: ", nRoundKeys);
  ...
  for (int i = 0; i < 240; i++) {
    ...
    for (int j = 0; j < 8; j++) {
      ...
      rk[i][j] = TFHEpp::tlweSymIntEncrypt<typename bkP::domainP>(
          (typename bkP::domainP::T)bin_str[j], bkP::domainP::alpha,
          sk->key.get<typename bkP::domainP>());
      // cout << endl;
    }
  }
  ...
  BENCHMARK("Encrypt Plaintext", {
    ...
    for (int i = 0; i < 16; i++) {
      ...
      for (int j = 0; j < 8; j++) {
        //  cout << bin_str[j]<<" ";
        cipher[i][j] = TFHEpp::tlweSymIntEncrypt<typename bkP::domainP>(
            (typename bkP::domainP::T)bin_str[j], bkP::domainP::alpha,
            sk->key.get<typename bkP::domainP>());
      }
      // cout <<endl;
  ...
  BENCHMARK("AES Transciphering", {
    // ======================================================================================
    // uncomment if you need to benchmark it
    // BENCHMARK("Add Round Key (round 0)", {
    CipherAddRoundKey(cipher, rk, 0);
    // });

    std::vector<std::vector<TFHEpp::TRGSWFFT<typename privksP::targetP>>>
        bootedTGSW;
    ...
    // ======================================================================================
    // for the 9 rounds of AES Transciphering
    for (int i = 1; i < 10; i++) {
      printer.PrintMessages("Round: ", i);

      // BENCHMARK("Circuit Bootstrapping (16*8)", {
      for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 8; j++)
          TFHEpp::SM4_CircuitBootstrappingFFT<iksP, bkP, privksP>(
              bootedTGSW[i][j], cipher[i][j], ek);
      }
      // });

      std::vector<TRLWE_1> lut_result(16);
      // BENCHMARK("Sbox Lookup Table", {
      for (int i = 0; i < 16; i++) {
      ...
      for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 8; j++) {
          TFHEpp::SampleExtractIndex<typename privksP::targetP>(
              Sbox_value[i][j], lut_result[i], j);
        }
      }
      ...
      // BENCHMARK("Identity keyswitch for one round", {
      for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 8; j++) {
          // level 1 -> level 0
          TFHEpp::IdentityKeySwitch<iksP>(B[i][j], Sbox_value[i][j],
                                          ek.getiksk<iksP>());
        }
      }
      // });

      // BENCHMARK("CipherShiftRows", {
      CipherShiftRows(cipher, B);
      // });

      // BENCHMARK("CipherMixColumns", {
      CipherMixColumns(cipher, consByte);
      // });

      // BENCHMARK("Add Round Key", {
      CipherAddRoundKey(cipher, rk, i);
      // });

      printer.PrintMessages("Round (", i, ") Cipher Add Round Key...");
      for (int i = 0; i < 16; i++) {
        int dec_hex = 0;
        int dec_bin[8];
        for (int j = 0; j < 8; j++) {
          // typename P::T a = TFHEpp::tlweSymIntDecrypt<typename
          // bkP::domainP>();
          dec_bin[j] = TFHEpp::tlweSymIntDecrypt<typename bkP::domainP>(
              cipher[i][j], sk->key.get<typename bkP::domainP>());
    ...
    printer.PrintMessages("Round (10) Cipher Add Round Key...");

    // BENCHMARK("Circuit bootstrapping (16 * 8) for one round", {
    for (int i = 0; i < 16; i++) {
      for (int j = 0; j < 8; j++)
        TFHEpp::SM4_CircuitBootstrappingFFT<iksP, bkP, privksP>(
            bootedTGSW[i][j], cipher[i][j], ek);
    ...
    for (int i = 0; i < 16; i++) {
      for (int j = 0; j < 8; j++) {
        TFHEpp::SampleExtractIndex<typename privksP::targetP>(Sbox_value[i][j],
                                                              lut_result[i], j);
      }
    }
