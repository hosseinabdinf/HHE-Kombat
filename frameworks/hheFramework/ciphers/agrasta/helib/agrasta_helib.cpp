...
std::vector<Ctxt> AGRASTA_128_HElib::HE_decrypt(std::vector<uint8_t>& ciphertexts, size_t bits) {
...
std::vector<uint8_t> AGRASTA_128_HElib::decrypt_result(std::vector<Ctxt>& ciphertexts) {
...
void AGRASTA_128_HElib::MultiplyWithGF2Matrix(const std::vector<block>& matrix, std::vector<Ctxt>& state) {
  std::vector<Ctxt> out;
...
void AGRASTA_128_HElib::MultiplyWithGF2Matrix(const mzd_t* matrix, std::vector<Ctxt>& state) {
...
void AGRASTA_128_HElib::addConstant(std::vector<Ctxt>& state, const block& constant) {
  NTL::ZZX p;
  std::vector<long> tmp(nslots, 0);
...
void AGRASTA_128_HElib::add(std::vector<Ctxt>& state, const std::vector<Ctxt>& key) {
  for (size_t i = 0; i < params.cipher_size_bits; i++) {
    state[i] += key[i];
  }
}
